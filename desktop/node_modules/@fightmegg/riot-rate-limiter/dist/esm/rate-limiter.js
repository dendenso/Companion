...
export const createRateLimiters = (rateLimits, opts) => {
    const { limits, counts } = rateLimits;
    const limitsArr = limits.split(",");
    const countsArr = counts.split(",");
    const rateLimiters = {};
    rateLimiters.limiters = limitsArr.map((limit, index) => new Bottleneck(createRateLimiterOptions(limit, countsArr[index], {
        ...opts,
        id: `${opts.id}_${index}`,
    })));
...
export const createRateLimitRetry = (limitType, retryAfterDefault, retryLimit) => (err, jobInfo) => {
